...
def run_af3_es_via_pipe(
    ...
    try:
        ...
        # Preserve the original ES behavior: NaN means that no AF3 seed
        # yielded a valid ES value.  XGBoost handles NaN as a missing value.
        # Infinity is not a valid ES value and remains an error.
        if math.isinf(value):
            raise ValueError(
                f"AF3 ES is infinite: {value}"
            )
...
def main():
    ...
    try:
        ...
        # Keep the original AF3-ES missing-value semantics: only Avg_ES
        # (feature index 36) may be NaN.  All other features must remain
        # finite, and infinity is never accepted.
        invalid = ~np.isfinite(features)
        if np.isnan(features[36]):
            invalid[36] = False

        if np.any(invalid):
            raise ValueError(
                "At least one internal feature other than AF3 ES is non-finite."
            )
