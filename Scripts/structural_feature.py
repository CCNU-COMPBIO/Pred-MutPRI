...
This module keeps the original feature calculations and paths, but it no longer
prints the six structural features. The top-level predictor imports
compute_structural_features() directly, so the feature vector stays in memory.

Expected flat layout:
    structural_feature.py
    FoldX/
    x3dna-dssr/
    <PDB>.pdb
    ...
