...
def atoms_of_residue(res):
    # Preserve all atoms present in the PDB, including hydrogen atoms.
...
def step3_compute_network_features(script_dir: Path, pdb_id: str, chain: str, mut: str) -> Path:
    ...
    # Exact model feature name. The value is the amino-node-only
    # NumberOfUndirectedEdges difference in the protein–nucleic-acid network.
    features_all["AverageShortestPathLength_Weightedinter_w"] = float(
        features_all.get("AverageShortestPathLength_NAinter_w", 0.0)
    )
    features_all["Δ_NumberOfUndirectedEdges_AA_inter"] = (
        compute_delta_NumberOfUndirectedEdges_NA_amino_only(base_dir, pdb_id)
    )
